...
// Counts player(s),exit(s),collectibles
...
// Check for invalid chars in map.ber
...
// Checks that map is rectangular and not square
void check_map_shape(t_game *game)
{
	size_t row_length;
	int i;

	row_length = ft_strlen(game->map[0]); // first row length
	if ((int)row_length == game->height)  // if 1st row length is == game height,the map is square
	{
	...
	while (i < game->height)
	{
		if (ft_strlen(game->map[i]) != row_length) // checks that all rows have same length
		{
...
void check_walls(t_game *game)
{
	...
	while (x < game->width)
	{
		if (game->map[0][x] != '1' || game->map[game->height - 1][x] != '1') // first and last rows
		{
	...
	while (y < game->height)
	{
		if (game->map[y][0] != '1' || game->map[y][game->width - 1] != '1') // first and last column
		{
...
// Checks that map has .ber format
int is_valid_map_extension(char *f)
{
	...
	while (f[i] != '\0')
		i++;
	if (f[i - 4] == '.' && f[i - 3] == 'b' && f[i - 2] == 'e' && f[i - 1] == 'r') // checks that map filename ends with .ber
	{
		i = 0;
		while (f[i] != '\0')
		{
			if (f[i] == '.' && f[i + 1] == 'b' && f[i + 2] == 'e' && f[i + 3] == 'r') // checks that there's only one .ber
				dot_count++;
			i++;
		}
		if (dot_count > 1)
